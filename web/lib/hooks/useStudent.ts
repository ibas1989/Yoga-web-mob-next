...
export function useStudent(studentId: string | null) {
  ...
  const loadStudent = useCallback(async () => {
    ...
    try {
      ...
      if (foundStudent) {
        ...
        const studentSessions = getSessionsForStudent(studentId).sort(
  ...
  const updateStudent = useCallback((updatedStudent: Student) => {
    ...
    const studentSessions = getSessionsForStudent(updatedStudent.id).sort(
      (a, b) => b.date.getTime() - a.date.getTime()
    );
    setSessions(studentSessions);
  }, []);
  ...
  useEffect(() => {
    ...
    const handleStudentUpdated = (event: CustomEvent) => {
      const { studentId: updatedStudentId, student: updatedStudent } =
        event.detail;
