...
export function testBalanceFormatting() {
  ...
  testBalances.forEach((balance) => {
    ...
    console.log(
      `Balance: ${balance} -> Formatted: ${formatted}, Integer: ${integer}`
    );
  });
 ...
 */
export function verifyNotesAssociation(
  student: Student,
  allStudents: Student[]
) {
  ...
  const duplicateNotes = otherStudentsNotes.filter((note) =>
    studentNoteIds.has(note.id)
  );

  if (duplicateNotes.length > 0) {
    console.error(
      `Found ${duplicateNotes.length} duplicate notes for student ${student.name}`
    );
    console.error(
      'Duplicate note IDs:',
      duplicateNotes.map((n) => n.id)
    );
    return false;
  }
...
export function testDuplicateNoteValidation() {
  ...
  const hasDuplicate = mockStudent.notes.some(
    (note) => note.name === duplicateName
  );
