...
export const saveStudent = (student: Student): void => {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('studentsUpdated', { detail: { student: studentToSave } })
    );
    window.dispatchEvent(
      new CustomEvent('studentUpdated', {
        detail: { studentId: student.id, student: studentToSave },
      })
    );
  }
};

export const addBalanceTransaction = (
  studentId: string,
  changeAmount: number,
  reason: string
): void => {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('balanceTransactionAdded', {
...
export const saveSession = (session: Session): void => {
  ...
  if (typeof window !== 'undefined') {
    if (isNewSession) {
      window.dispatchEvent(
        new CustomEvent('sessionCreated', { detail: { session } })
      );
    } else {
      window.dispatchEvent(
    ...
    window.dispatchEvent(
      new CustomEvent('sessionChanged', { detail: { session } })
    );
  }
};
...
export const deleteSession = (sessionId: string): void => {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('sessionDeleted', { detail: { sessionId } })
    );
    window.dispatchEvent(
      new CustomEvent('sessionChanged', { detail: { sessionId } })
    );
  }
};
...
// Student Notes
export const checkDuplicateNoteName = (
  studentId: string,
  noteName: string
): boolean => {
  const students = getStudents();
...
export const cancelSession = (sessionId: string): void => {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('sessionCancelled', { detail: { sessionId, session } })
    );
    window.dispatchEvent(
      new CustomEvent('sessionChanged', { detail: { session } })
    );
...
export const completeSession = (
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('sessionUpdated', { detail: { sessionId, session } })
    );
    window.dispatchEvent(
      new CustomEvent('sessionCompleted', { detail: { sessionId, session } })
    );
    window.dispatchEvent(
      new CustomEvent('sessionChanged', { detail: { session } })
    );
...
export const completeSessionTranslated = (
  ...
  confirmedAttendeeIds.forEach((studentId) => {
    ...
    const sessionTypeEn =
      session.sessionType === 'team' ? 'team' : 'individual';
    const sessionTypeRu =
      session.sessionType === 'team' ? 'командное' : 'индивидуальное';
    ...
    addBalanceTransactionBilingual(
      studentId,
      -sessionDeduction,
      reasonEn,
      reasonRu
