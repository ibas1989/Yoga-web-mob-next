...
export const BackupManager: React.FC<BackupManagerProps> = ({
  ...
  const [googleDriveAuthState, setGoogleDriveAuthState] =
    useState<GoogleDriveAuthState>({ isAuthenticated: false });
  ...
  const getNextBackupTime = () => {
    ...
    const nextBackup = new Date(
      lastBackup.getTime() + autoBackupConfig.interval * 60 * 60 * 1000
    );
    return nextBackup;
  };
      ...
      if (autoBackupConfig.googleDriveConfig) {
        try {
          const googleDriveService = initializeGoogleDriveService(
            autoBackupConfig.googleDriveConfig
          );
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (validation.valid) {
          ...
          showMessage(
            'error',
            t('backup.importBackup.validationFailed', {
      ...
      if (
        ...
        try {
          ...
          if (handle && typeof handle.getFileHandle === 'function') {
            ...
            showMessage(
              'success',
              t('backup.autoBackup.enableFolderSuccessWithBackup')
            );
          } else {
          ...
          showMessage(
            'info',
            t('backup.autoBackup.enableFolderSuccessNoBackup')
          );
        } finally {
          setIsLoading(false);
        }
    ...
    try {
      if (directoryHandle) {
        ...
        try {
          ...
          const fileHandle = await directoryHandle.getFileHandle(tempFileName, {
            create: true,
          });
          const writable = await fileHandle.createWritable();
          ...
          try {
            ...
            showMessage(
              'success',
              t('backup.autoBackup.folderOpened', { path: selectedFolderPath })
            );
          } catch (downloadError) {
            // Approach 2: Show folder path with instructions
            showMessage(
              'info',
              t('backup.autoBackup.folderLocationWithInstructions', {
          ...
          showMessage(
            'info',
            t('backup.autoBackup.folderLocation', { path: selectedFolderPath })
          );
        }
      } else {
  ...
  const handleUploadToGoogleDrive = async () => {
    ...
    try {
      ...
      showMessage(
        'error',
        t('backup.googleDrive.uploadError', {
          error: error instanceof Error ? error.message : 'Unknown error',
        })
      );
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 mt-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-gray-600">
            {stats.students} {t('backup.students')} • {stats.sessions}{' '}
            {t('backup.sessions')}
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">
                {t('backup.exportBackup.title')}
              </h3>
              <p className="text-sm text-gray-600">
                {t('backup.exportBackup.description')}
              </p>
            </div>
          </div>
          <Button
          ...
          >
            {isLoading
              ? t('backup.exportBackup.exporting')
              : t('backup.exportBackup.button')}
          </Button>
        </Card>
        ...
        <Card className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">
                {t('backup.importBackup.title')}
              </h3>
              <p className="text-sm text-gray-600">
                {t('backup.importBackup.description')}
              </p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            >
              {isLoading
                ? t('backup.importBackup.importing')
                : t('backup.importBackup.button')}
            </Button>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                {autoBackupConfig.enabled
                  ? t('backup.autoBackup.enabled')
                  : t('backup.autoBackup.disabled')}
              </span>
            </div>
            <div className="text-xs text-gray-600 space-y-1">
              <div>
                •{' '}
                {t('backup.autoBackup.backsUpEvery', {
                  hours: autoBackupConfig.interval,
                })}
              </div>
              <div>
                •{' '}
                {t('backup.autoBackup.keepsBackups', {
                  count: autoBackupConfig.maxBackups,
                })}
              </div>
              {autoBackupConfig.enabled &&
                ...
                getNextBackupTime() && (
                  <div className="text-blue-600 font-medium">
                    {t('backup.autoBackup.nextBackup', {
                      time: getNextBackupTime()!.toLocaleString(),
                    })}
                  </div>
                )}
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">
                {t('backup.autoBackup.enableToggle')}
              </span>
              <button
            ...
            {autoBackupConfig.enabled && selectedFolderPath && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">
                  {t('backup.autoBackup.backupLocation')}
                </label>
        ...
        <Card className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">
                {t('backup.googleDrive.title')}
              </h3>
              <p className="text-sm text-gray-600">
                {t('backup.googleDrive.description')}
              </p>
            </div>
          </div>
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                {googleDriveAuthState.isAuthenticated
                  ? t('backup.googleDrive.connected', {
                      email: googleDriveAuthState.userEmail || '',
                    })
                  : t('backup.googleDrive.notConnected')}
              </span>
            </div>
            {googleDriveAuthState.isAuthenticated && (
              <div className="text-xs text-gray-600 space-y-1">
                <div>
                  •{' '}
                  {t('backup.googleDrive.backupFiles', {
                    count: googleDriveFiles.length,
                  })}
                </div>
                <div>
                  •{' '}
                  {t('backup.googleDrive.autoBackup', {
                    status:
                      autoBackupConfig.storageLocation === 'google_drive'
                        ? t('backup.googleDrive.enabled')
                        : t('backup.googleDrive.disabled'),
                  })}
                </div>
              </div>
            )}
          ...
          <div className="space-y-3">
            {!googleDriveAuthState.isAuthenticated ? (
              ...
              <div className="space-y-2">
                ...
                >
                  {isLoading
                    ? t('backup.googleDrive.uploading')
                    : t('backup.googleDrive.uploadButton')}
                </Button>
                <Button
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    {backup.studentCount} {t('backup.students')} •{' '}
                    {backup.sessionCount} {t('backup.sessions')}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 p-3 rounded-lg">
              <h4 className="text-sm font-medium text-blue-900 mb-2">
                {t('backup.googleDrive.setupInstructions')}
              </h4>
            ...
            <div className="flex space-x-2">
              ...
              >
                {isLoading
                  ? t('backup.googleDrive.connecting')
                  : t('backup.googleDrive.connect')}
              </Button>
            </div>
          </div>
        </DialogContent>
