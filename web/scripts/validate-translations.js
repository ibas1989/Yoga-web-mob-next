...
const enTranslations = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../lib/i18n/en.json'), 'utf8')
);
const ruTranslations = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../lib/i18n/ru.json'), 'utf8')
);
...
function findEmptyTranslations(obj, prefix = '') {
  ...
  for (const key in obj) {
    if (typeof obj[key] === 'object' && obj[key] !== null) {
      emptyKeys = emptyKeys.concat(
        findEmptyTranslations(obj[key], prefix ? `${prefix}.${key}` : key)
      );
...
function findHardcodedStrings(obj, prefix = '') {
  ...
  for (const key in obj) {
    if (typeof obj[key] === 'object' && obj[key] !== null) {
      hardcodedKeys = hardcodedKeys.concat(
        findHardcodedStrings(obj[key], prefix ? `${prefix}.${key}` : key)
      );
