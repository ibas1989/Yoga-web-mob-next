...
function isSessionEndTimePassed(session) {
  ...
  try {
    ...
    log(
      `Error checking session end time for session ${session.id}: ${error.message}`
    );
    return false;
  }
}
...
function getSessions() {
  try {
    ...
    log(
      'Note: This script simulates the check. In production, this would need to run in a browser context or use server-side storage.'
    );
